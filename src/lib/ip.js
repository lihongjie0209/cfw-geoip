...
const validate = (ip) => {
    ...
    if (ip.indexOf(':') === -1) {
        ...
        for (const part of parts) {
            if (part && part.split(':').length > 8)
                return false; // More than 8 groups is invalid
            const hexParts = part ? part.split(':') : [];
            for (const hexPart of hexParts) {
                if (hexPart && !/^[0-9a-fA-F]{1,4}$/.test(hexPart))
                    return false; // Invalid hex group
            }
        }
        return true;
    }
